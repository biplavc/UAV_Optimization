...
def create_graph_1(user_coordinates, grid_coordinates):
    ...
    drones_needed_MDS, n = custom_mds(J, drone_ids, user_ids, position_dict_1) # drones_needed_MDS is a list
    assert (n == 0) # all users have to be removed as removed means under coverage

    H = G.__class__() # deep copy
    H.add_nodes_from(G)
    H.add_edges_from(G.edges)
    drones_needed_random, m = random_selection(H, drone_ids_1, user_ids_1, position_dict_2) # drones_needed_random is a list
    assert (m == 0) # all users have to be removed as removed means under coverage

    # I = G.__class__() # deep copy
    # I.add_nodes_from(G)
    # I.add_edges_from(G.edges)
    # drones_needed_MCDS, m = random_selection(I, drone_ids_2, user_ids_2, position_dict)
    # assert (m == 0) # all users have to be removed as removed means under coverage
    ...
    return (len(drones_needed_random), len(drones_needed_MDS))
...
def custom_mds(G, drone_ids, user_ids, position_dict):
    ...
    return ((selected_nodes_list), len(user_ids)) #, G.number_of_nodes())
...
def construct_graph(user_coordinates, drone_coordinates): # no manipulation of lists, only construct the graph between drone-users based on drone's ground coverage
